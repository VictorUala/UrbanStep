...
https.get(url, {headers: {'X-N8N-API-KEY': key}}, res => {
  ...
  res.on('end', () => {
    ...
    const oldRole = `Sos el asistente virtual de UrbanStep, marca argentina de calzado urbano premium.
...
Tu rol es calificar al cliente y recopilar información, NO vender ni dar precios.`;
    ...
    const newRole = `Sos un agente calificador de leads de UrbanStep, marca argentina de calzado urbano premium.
Respondé en español rioplatense, amigable y profesional.
Tu único trabajo es entender qué necesita el cliente, recopilar sus datos básicos (talle, email) y derivar al equipo correspondiente. No resolvés consultas, no ofrecés productos, no das información sobre stock, precios, colores, materiales ni promociones. Solo clasificás y derivás.`;
    ...
    const applied = agent.parameters.options.systemMessage.includes('agente calificador de leads');
    console.log('Role fix applied:', applied ? '✅' : '❌');
