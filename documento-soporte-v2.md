...
| Campo | Descripción |
|---|---|
| `id` | Identificador único interno (clave primaria, UUID) |
| `nombre` | Nombre completo del cliente |
| `email` | Email real (null si solo vino por Telegram) |
| `telegram_id` | ID de cuenta Telegram |
| `canal_origen` | Canal de primer contacto (`telegram`, `web`, etc.) |
| `historial_compras` | Array de compras previas |
...
| `direccion` | `entrada` (cliente) o `salida` (bot) |
| `canal` | Canal del mensaje (`telegram`, `web`, `nps-form`) |
| `timestamp` | Fecha y hora del mensaje |

### `conversaciones`
Análisis IA de cada conversación.
...
| `intencion` | Nivel de intención de compra |
| `etapa_funnel` | Etapa del funnel de ventas |
| `objecion_principal` | Objeción principal detectada |
| `resumen` | Resumen de la conversación |
| `prioridad` | Nivel de prioridad |
| `accion_recomendada` | Acción sugerida por la IA |
| `email_detectado` | Email mencionado por el cliente en la conversación |
| `seguimiento_enviado` | Si ya se envió mensaje de seguimiento automático |
| `timestamp` | Fecha del análisis |

### `compras_clientes`
Historial comercial para detectar clientes recurrentes y enriquecer el análisis.
...
| `canal_venta` | Canal donde se realizó la venta |
...
| `id` | Identificador único |
| `cliente_id` | Referencia al cliente |
| `workflow_id` | Identificador del flujo (`v2-parallel`, `seguimiento-automatico`) |
| `rama` | Rama ejecutada (`conversacion`, `compra`, `queja`, `nps`, etc.) |
| `estado` | `exito`, `error` o `fallback` |
| `detalle` | JSON con datos adicionales (tokens, sentimiento, canal, etc.) |
...
|---|---|
| `id` | Identificador único |
| `tipo` | Tipo de asset (`promo_recurrente`, `catalogo_lookbook`, `logo`) |
| `descripcion` | Descripción del contenido |
| `url` | URL pública de la imagen (actualizable sin tocar el workflow) |
| `activo` | Si el asset está activo |
