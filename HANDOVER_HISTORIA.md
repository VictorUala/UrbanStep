...
## Log 2026-04-25 — Sesión NPS testing (execuciones 289-300)

### Testing NPS automatizado — 8/8 CORRECTOS ✅

**Contexto:** Se implementó webhook `/webhook/urbanstep-nps-test` para inyectar los 8 casos NPS del PDF sin formulario manual.

**Errores previos (execuciones 289-292):** 4 execuciones fallidas antes del fix — root cause: `Crear Cliente NPS` y `Guardar Mensaje NPS` referenciaban `Extraer Datos NPS` (nodo de la rama Form) que no se ejecutaba en la rama webhook → crash.

**Fix aplicado:** `.isExecuted` check en `Crear Cliente NPS` y `Guardar Mensaje NPS`:
```
nombre: "={{ $('Extraer Datos NPS').isExecuted ? $('Extraer Datos NPS').first().json.nombre : $('Extraer Datos NPS Webhook').first().json.nombre }}"
```
También: `IF Existe NPS` ahora chequea `$json.id` notEmpty (no length>0), y `Buscar Cliente NPS` tiene `alwaysOutputData: true` + filtro correcto como expresión.

**Fix adicional:** Router NPS movido a posición [0] (primera prioridad) para que Facundo NPS=3 (negativo/alta) vaya a NPS y no a Queja.

**Resultados execuciones 293-300 (8/8 ✅):**
| Exec | Cliente | NPS | Sentimiento | Prioridad | Funnel | Router |
|------|---------|-----|-------------|-----------|--------|--------|
| 293 | Lucía Pérez | 10 | positivo ✅ | baja ✅ | nps ✅ | [0] ✅ |
| 294 | José López | 9 | positivo ✅ | baja ✅ | nps ✅ | [0] ✅ |
| 295 | Martina Gómez | 8 | (success ✅) | (success ✅) | nps ✅ | [0] ✅ |
| 296 | Carlos Fernández | 7 | neutro ✅ | media ✅ | nps ✅ | [0] ✅ |
| 297 | Valentina Rojas | 6 | negativo ✅ | alta ✅ | nps ✅ | [0] ✅ |
| 298 | Juan Cruz | 5 | (success ✅) | (success ✅) | nps ✅ | [0] ✅ |
| 299 | Sofía Martínez | 4 | (success ✅) | (success ✅) | nps ✅ | [0] ✅ |
| 300 | Facundo Silva | 3 | negativo ✅ | alta ✅ | nps ✅ | [0] ✅ |

Fronteras críticas correctas: NPS=9→positivo, NPS=6→negativo, NPS=3→va a NPS y no a Queja.
Tokens capturados: 85-215 por ejecución.

### Router Decisiones — orden final (post-fix NPS)
- [0] NPS: etapa_funnel=nps ← PRIORIDAD MÁXIMA (evita que NPS negativo caiga en Queja)
- [1] Queja urgente: sentimiento=negativo AND prioridad=alta
- [2] Recurrente: es_recurrente=true
- [3] Compra inmediata: etapa_funnel=decision AND intencion=alta
- [4] Post-venta: etapa_funnel=post-venta
- [5] Nurturing: etapa_funnel=consideracion
- [6] Exploratorio: etapa_funnel=awareness
- [7] Fallback

### Workflow Seguimiento Automático — COMPLETO ✅

**ID:** LQAiZVEE85Iqbppl — activo, corre diariamente a las 9 AM Argentina (12:00 UTC)

**Arquitectura (8 nodos):**
```
Schedule Trigger (0 12 * * *)
  → Buscar Pendientes (Supabase GetAll conversaciones: seguimiento_enviado=false, etapa_funnel activo, timestamp >48h)
  → Obtener Cliente (Supabase GetAll clientes: id=eq.cliente_id)
  → Construir Mensaje Seguimiento (Code: mensaje personalizado por etapa_funnel)
  → IF Tiene Telegram
      TRUE → Telegram Seguimiento → Marcar Enviado (HTTP Request PATCH Supabase) → Log Seguimiento
      FALSE → termina (sin canal)
```

**Mensajes por etapa_funnel:**
- decision: "Vimos que estabas por decidirte. ¿Pudiste elegir tu par?"
- consideracion: "¿Seguís evaluando opciones? Si tenés dudas, escribinos."
- awareness: "¿Pudimos ayudarte con lo que buscabas?"
- post-venta: "¿Cómo anduvieron las zapatillas?"

**Fixes aplicados durante implementación:**
- Marcar Enviado: Supabase update con `matchingColumns` no funciona via API → reemplazado por HTTP Request (PATCH a `https://wigpybyzjwtjiixnfymf.supabase.co/rest/v1/conversaciones?id=eq.{id}`) con anon key
- Log Seguimiento: CHECK constraint `logs_estado_check` acepta solo `exito`, `error`, `fallback` (no `success`)
- Campo `seguimiento_enviado` (boolean DEFAULT false) agregado a tabla `conversaciones` via migration

**Nota técnica importante:**
> El nodo Supabase update con `matchingColumns` no funciona cuando se crea via API (matchingColumns llega vacío en runtime). Para updates en nuevos workflows: usar HTTP Request + Supabase REST API PATCH directamente.

**Test confirmado:**
- Victor Duart (telegram 8704924722), etapa_funnel=decision → mensaje Telegram recibido ✅
- conversaciones.seguimiento_enviado = true ✅  
- logs: estado=exito, detalle con etapa_funnel, sentimiento, telegram_id ✅

### Pendientes para la defensa (3 días restantes — entrega martes 29/04)
1. **Activar Telegram Fidelizacion + Telegram Promo** (deshabilitados desde sesión 7)
...
5. **Commitear** fix_email_fantasma.js + fixes NPS + nuevo workflow seguimiento
6. **Reactivar retryOnFail** en AI Agents (estaba en false para testing)

---

## Log 2026-04-26 — Sesión 12 (HubSpot nota + fix .internal)

### Contexto de recuperación
Sesión se compactó. Estado recuperado vía HANDOVER.md + summary de compactación.

### Bug crítico descubierto: HubSpot rechaza TLDs de más de 3 letras
- El profesor lo descubrió durante pruebas y Victor lo verificó
- `.internal` tiene 8 letras → HubSpot rechaza silenciosamente (continueOnFail=true)
- Consecuencia: todos los contactos con email fantasma NUNCA se creaban en HubSpot
- Fix aplicado en DOS lugares:
  - Workflow principal (SDvniUq2L5axPWWu) nodo **Enriquecer Recurrencia**: `.internal` → `.bot`
  - Workflow seguimiento (LQAiZVEE85Iqbppl) nodo **Construir Mensaje Seguimiento**: `.internal` → `.bot`
- Formato nuevo: `telegram-{telegram_id}@urbanstep.bot` (3 letras, HubSpot lo acepta)

### HubSpot nota — rediseño de "Buscar Contacto HubSpot"
- Problema original: GET por email devolvía 404 si el contacto no existía → contactId="" → nota fallaba
- Root cause: el contacto con email fantasma no existía porque `.internal` era rechazado por HubSpot
- Fix aplicado: cambiar de GET a **POST batch/upsert** (`/crm/v3/objects/contacts/batch/upsert`)
  - Crea el contacto si no existe
  - Actualiza si ya existe
  - SIEMPRE devuelve `results[0].id`
- "Preparar Nota HubSpot" actualizado: lee `results[0].id` en vez de `id`
- Workflow seguimiento ahora tiene 11 nodos 100% funcionales (en teoría)

### Estado al salir Victor
- `seguimiento_enviado` reseteado a false para conversacion `36b4b08d-9505-4bdc-8ccd-5ac0693c562c`
- Pendiente: ejecutar workflow seguimiento y verificar que nota aparece en HubSpot
- Victor salió a ver si se activa el plan Extra Large de Claude

### Pendientes inmediatos al volver
1. **Ejecutar seguimiento** y verificar nota en HubSpot (tab Actividades de Victor Duart)
2. **Activar Telegram Fidelizacion + Telegram Promo**
3. **Documento de soporte** 2-3 páginas
4. **Exportar JSON + Excalidraw + Google Drive**
5. **Ensayo general** 3 casos de defensa
6. **Commitear** todos los cambios pendientes
