...
https.get(url, {headers: {'X-N8N-API-KEY': key}}, res => {
  ...
  res.on('end', () => {
    ...
    // Replace PASO 1 to add the post-handoff greeting rule
    const oldPaso1 = `PASO 1 — ¿Estoy en una conversación abierta o es una nueva?
...
- Si NO fue handoff → seguís en la misma conversación → NO saludes, respondé directo`;
    ...
    const newPaso1 = `PASO 1 — ¿Estoy en una conversación abierta o es una nueva?
Fijate si tu último mensaje en el historial fue un handoff (despedida, "te van a contactar", "necesitás algo más").
- Si fue handoff → este mensaje del cliente es una conversación NUEVA → saludá normalmente y preguntá en qué podés ayudar. NO hagas referencia a temas de sesiones anteriores a menos que el cliente los mencione primero.
- Si NO fue handoff → seguís en la misma conversación → NO saludes, respondé directo`;

    agent.parameters.options.systemMessage = prompt.replace(oldPaso1, newPaso1);

    const applied = agent.parameters.options.systemMessage.includes('NO hagas referencia a temas de sesiones anteriores');
